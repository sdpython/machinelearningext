...
using DropNaN = Scikit.ML.PipelineTransforms.DropNaN;

[assembly: LoadableClass(DropNaN.Summary, typeof(DropNaN), typeof(DropNaN.Arguments), typeof(SignatureDataTransform),
    DropNaN.FriendlyName, DropNaN.ShortName, "DropMissingValueFilter", "DropMissingValue")]

[assembly: LoadableClass(DropNaN.Summary, typeof(DropNaN), null, typeof(SignatureLoadDataTransform),
    DropNaN.FriendlyName, DropNaN.LoaderSignature, "DropMissingFeatureFilter")]

namespace Scikit.ML.PipelineTransforms
{
    public sealed class DropNaN : FilterBase
    {
        ...
        public const string FriendlyName = "Drop NA";
        public const string ShortName = "DropNaN";

        public const string LoaderSignature = "DropMissingValueFilter";
        private static VersionInfo GetVersionInfo()
        {
            return new VersionInfo(
                modelSignature: "DROPNAFL",
                verWrittenCur: 0x00010001, // Initial
                verReadableCur: 0x00010001,
                verWeCanReadBack: 0x00010001,
                loaderSignature: LoaderSignature,
                loaderSignatureAlt: "DropMissingFeatureFilter",
                loaderAssemblyName: typeof(DropNaN).Assembly.FullName);
        }
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="DropNaN"/>.
        /// </summary>
        ...
        public DropNaN(IHostEnvironment env, IDataView input, bool complement = Defaults.Complement, params string[] columns)
            : this(env, new Arguments() { Columns = columns, Complement = complement }, input)
        {
        }

        public DropNaN(IHostEnvironment env, Arguments args, IDataView input)
        ...
        public DropNaN(IHost host, ModelLoadContext ctx, IDataView input)
            : base(host, input)
        {
        ...
        public static DropNaN Create(IHostEnvironment env, ModelLoadContext ctx, IDataView input)
        {
            ...
            ctx.CheckAtModel(GetVersionInfo());
            return h.Apply("Loading Model", ch => new DropNaN(h, ctx, input));
        }
        ...
        private sealed class Cursor : LinkedRowFilterCursorBase
        {
            private abstract class Value
            {
                ...
                private static ValueOne<T> CreateOne<T>(Cursor cursor, ColInfo info)
                {
                    ...
                    var hasBad = Microsoft.ML.Data.Conversion.Conversions.Instance.GetIsNAPredicate<T>(info.Type);
                ...
                private static ValueVec<T> CreateVec<T>(Cursor cursor, ColInfo info)
                {
                    ...
                    var hasBad = Microsoft.ML.Data.Conversion.Conversions.Instance.GetHasMissingPredicate<T>((VectorType)info.Type);
            ...
            private readonly DropNaN _parent;
            private readonly Value[] _values;

            public Cursor(DropNaN parent, DataViewRowCursor input, bool[] active)
